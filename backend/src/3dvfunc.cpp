      ...
      if (line.size() > 1 && line[0] == 'v' && line[1] == ' ') {
        ...
        for (size_t i = 0; i < line.size(); i++) {
          if (line[i] == ' ' || i == line.size() - 1) {   // where to write
            ...
            if (i > 0 && line[i] == '/' &&
                line[i - 1] == '/') { // check for v//vn format
          ...
          if (x > 0) { // we are ordering here based on winding. The normal
            ...
            }
            {
              ...
              while (vindices.size() >= 3) {
                if (reflex.empty() == 1) { //O(n) check for reflex vertices, if there are none, then we can be sure that all the triangles are valid,\
                  ...
                  if (reflex.find(*firstelement) != reflex.end()) {//we can't cast a triangle with a reflex vertex
                    firstelement++;
                    nextnext++;
                    prev1++;
                    prev2++;
                    ...
                    if (reflexelement == reflex.size()) {
                      ...
                      for (size_t j = 0; j < reflex.size(); j++) {
                        ...
                        if (valid == true) { // we exclude go step 1 forward in
                      ...
                      if (valid == false) {//in the case of the triangle being valid
                        cs.triangles.emplace_back(
                            std::array<long unsigned int, 3>{
                                v0, cs.fvi[i].ver[*firstelement].v - 1, v1});
                        firstelement = vindices.erase(firstelement);
  ...
  void MoellerTrumbore(uint32_t vecnum) {
    ...
    for (size_t i = 0; i < cs.usable.size(); i++) {
      ...
      std::array<float, 3> v0 = {
          cs.vxc[cs.triangles[j][0]] - cs.stpxcoords[vecnum],
          cs.vyc[cs.triangles[j][0]] - cs.stpycoords[vecnum],
          cs.vzc[cs.triangles[j][0]] - cs.stpzcoords[vecnum]};
      std::array<float, 3> v1 = {
          cs.vxc[cs.triangles[j][1]] - cs.stpxcoords[vecnum],
          cs.vyc[cs.triangles[j][1]] - cs.stpycoords[vecnum],
          cs.vzc[cs.triangles[j][1]] - cs.stpzcoords[vecnum]};
      std::array<float, 3> v2 = {
          cs.vxc[cs.triangles[j][2]] - cs.stpxcoords[vecnum],
          cs.vyc[cs.triangles[j][2]] - cs.stpycoords[vecnum],
          cs.vzc[cs.triangles[j][2]] - cs.stpzcoords[vecnum]};
      ...
      facedotprod = (cs.xcoords[vecnum] - cs.stpxcoords[vecnum]) * normal[0] +
                    (cs.ycoords[vecnum] - cs.stpycoords[vecnum]) * normal[1] +
                    (cs.zcoords[vecnum] - cs.stpzcoords[vecnum]) * normal[2];
      if (std::fabs(facedotprod) < 1e-6)
        continue; // Ray is parallel to triangle
      float t =
          -(-(normal[0] * v0[0] + normal[1] * v0[1] + normal[2] * v0[2])) /
          (normal[0] * (cs.xcoords[vecnum] - cs.stpxcoords[vecnum]) +
           normal[1] * (cs.ycoords[vecnum] - cs.stpycoords[vecnum]) +
           normal[2] * (cs.zcoords[vecnum] - cs.stpzcoords[vecnum]));
      if (t < 0 || t > 1) {
        std::cerr << "Vector meets the plane outside \n";
        continue;
      }
      std::array<float, 3> P = {
          t * (cs.xcoords[vecnum] - cs.stpxcoords[vecnum]),
          t * (cs.ycoords[vecnum] - cs.stpycoords[vecnum]),
          t * (cs.zcoords[vecnum] - cs.stpzcoords[vecnum])};
